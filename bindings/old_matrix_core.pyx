...
"""
This is mostly a copy of ../matrix/bindings/python/rgbmatrix/core.pyx
But the Classes are renamed like Canvas->PyCanvas to discern between the
C++ canvas and the one created here (for use with python).
Also, the cppinc module is not needed anymore.
"""
...
cdef class PyCanvas:
    ...
    def SetPixelsPillow(self, int xstart, int ystart, int width, int height, image):
        cdef FrameCanvas* my_canvas = <FrameCanvas*>self.__getCanvas()
...
cdef class PyFrameCanvas(PyCanvas):
    ...
    def Fill(self, uint8_t red, uint8_t green, uint8_t blue):
        (<FrameCanvas*>self.__getCanvas()).Fill(red, green, blue)

    def Clear(self):
        (<FrameCanvas*>self.__getCanvas()).Clear()

    def SetPixel(self, int x, int y, uint8_t red, uint8_t green, uint8_t blue):
        (<FrameCanvas*>self.__getCanvas()).SetPixel(x, y, red, green, blue)


    property width:
        def __get__(self): return (<FrameCanvas*>self.__getCanvas()).width()

    property height:
        def __get__(self): return (<FrameCanvas*>self.__getCanvas()).height()

    property pwmBits:
        def __get__(self): return (<FrameCanvas*>self.__getCanvas()).pwmbits()
        def __set__(self, pwmBits): (<FrameCanvas*>self.__getCanvas()).SetPWMBits(pwmBits)

    property brightness:
        def __get__(self): return (<FrameCanvas*>self.__getCanvas()).brightness()
        def __set__(self, val): (<FrameCanvas*>self.__getCanvas()).SetBrightness(val)


cdef class PyRGBMatrixOptions:
    def __cinit__(self):
        self.__options = Options()
        self.__runtime_options = RuntimeOptions()
...
cdef class PyRGBMatrix(PyCanvas):
    def __cinit__(self, int rows = 0, int chains = 0, int parallel = 0,
        ...
        self.__matrix = CreateMatrixFromOptions(options.__options,
    ...
    def SwapOnVSync(self, PyFrameCanvas newFrame):
...
cdef __createFrameCanvas(FrameCanvas* newCanvas):
    canvas = PyFrameCanvas()
