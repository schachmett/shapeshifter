...
def main():
    ...
    while True:
        ...
        for i in range(0, 18, 3):
            send_dict(ID=teams[i  ], command="target", position=pos1, duration=2500)
            send_dict(ID=teams[i+1], command="target", position=pos2, duration=2500)
            send_dict(ID=teams[i+2], command="target", position=pos3, duration=2500)
            time.sleep(3)

            send_dict(ID=teams[i  ], command="target", position=after1, duration=2500)
            send_dict(ID=teams[i+1], command="target", position=after2, duration=2500)
            send_dict(ID=teams[i+2], command="target", position=after3, duration=2500)
        time.sleep(3)
